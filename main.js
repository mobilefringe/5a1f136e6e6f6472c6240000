...
require(['Vue', 'vuex', 'vue2-filters', 'vue_router', 'routes', 'datastore', 'vue-i18n', 'locales', 'moment', 'vue-meta', "moment-timezone", "vue-moment",'vue-social-sharing','vue-select', "vue!search-component", 'vue-fullcalendar'], function (Vue, Vuex, Vue2Filters, VueRouter, appRoutes, store, VueI18n, messages, moment, Meta,, tz, VueMoment,SocialSharing, VueSelect, SearchComponent, FullCalendar) {
    Vue.use(Meta);
    Vue.use(VueRouter);
    Vue.use(Vue2Filters);
    Vue.use(VueI18n);
  ...
  const vm = new Vue({
    ...
    created() {
      this.$store.dispatch('initializeApi', {site: "canyoncrest", version: "v4"});
